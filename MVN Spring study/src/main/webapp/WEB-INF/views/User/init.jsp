...
<body>
	<form id="init" action="" method="post">
		...
		<table>
			...
			<tr>
				<td><b>성 명 </b>:</td>
				<td><input type="text" id="DELIVNAME" name="DELIVNAME"
					placeholder="성명을 입력하세요." required autofocus></td>
			</tr>
			<tr>
				<td><b>관 계 </b>:</td>
				<td><select id="RELCD" name="RELCD">
			...
			<tr>
				<td><b>우편번호 </b>:</td>
				<td><input type="number" id="ADDRCD" name="ADDRCD"
				    min="1" max="99999" placeholder="우편번호" required></td>
			</tr>
			<tr>
				<td><b>주 소 </b>:</td>
			...
			<tr>
				<td>휴대전화 :</td>
				<td><input type="tel" id="MOBILETELNO" name="MOBILETELNO"
					placeholder="휴대전화" pattern="[0-9]{2,3}[0-9]{3,4}[0-9]{3,4}"
					maxlength="11"></td>
			</tr>
			<tr>
				<td>집 전 화 :</td>
				<td><input type="tel" id="HOMETELNO" name="HOMETELNO"
					placeholder="집전화" pattern="[0-9]{2,3}[0-9]{3,4}[0-9]{3,4}"
					maxlength="11"></td>
			</tr>
			<tr>
				<td>사용 여부 :</td>
...
<script>
	function save() {
		...
		if ($MOBILETELNO != null || $MOBILETELNO != '') {
			...
			var res = patt.test($MOBILETELNO);

			if (!patt.test($MOBILETELNO)) {
				alert("휴대 전화번호를 정확히 입력하여 주십시오.");
				$('#MOBILETELNO').focus();
				return;
			}
		}
		if ($HOMETELNO != null || $HOMETELNO != '') {
			var patt = new RegExp("[0-9]{2,3}[0-9]{3,4}[0-9]{3,4}");
			var res = patt.test($HOMETELNO);

			if (!patt.test($HOMETELNO)) {
				alert("집 전화번호를 정확히 입력하여 주십시오.");
				$('#HOMETELNO').focus();
				return;
			}
		}
